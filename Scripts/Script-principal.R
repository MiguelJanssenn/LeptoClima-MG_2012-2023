...
População <- read_csv("IBGE-população-estimada.csv", 
...
#Datasus
LEPTMG <- read_csv("SINAN-leptospirose.csv", col_types = cols(DT_NOTIFIC = col_date(format = "%Y-%m-%d"), 
...
#INMET
INMET <- read_csv("BDMEP-precipitacao.csv", 
                  col_types = cols(N = col_number(), Ano = col_number(), 
                                   Data = col_date(format = "%Y-%m-%d"), 
                                   `Precipitação mensal (mm)` = col_number(), 
                                   M = col_character(), Mes = col_character()))
